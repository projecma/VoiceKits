...
import com.example.voicemaster.speech.setting.VoiceReadSettings;
...
public class VoiceRead extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mSharedPreferences = getSharedPreferences(VoiceReadSettings.PREFER_NAME, Activity.MODE_PRIVATE);
...
//			this.showTip( "创建对象失败，请确认 libmsc.so 放置正确，\n 且有调用 createUtility 进行初始化" );
		...
		case R.id.image_tts_set:
			Intent intent = new Intent(VoiceRead.this, VoiceReadSettings.class);
...
//        		showTip("初始化失败,错误码："+code+",请点击网址https://www.xfyun.cn/document/error-code查询解决方案");
	...
	private SynthesizerListener mTtsListener = new SynthesizerListener() {
		...
		public void onSpeakBegin() {
			//showTip("开始播放");
			Log.d(VoiceRead.TAG,"开始播放："+ System.currentTimeMillis());
		}
...
public void onCompleted(SpeechError error) {
	...
	private String getResourcePath(){
		...
		//发音人资源
		if(mEngineType.equals(SpeechConstant.TYPE_XTTS)){
			tempBuffer.append(ResourceUtil.generateResourcePath(this, RESOURCE_TYPE.assets, type+"/"+ VoiceRead.voicerXtts+".jet"));
		}else {
			tempBuffer.append(ResourceUtil.generateResourcePath(this, RESOURCE_TYPE.assets, type + "/" + VoiceRead.voicerLocal + ".jet"));
		}
